...
def mk_graph_data(df):
    ...
    for idx, group in groups:
        if final_df is None:
            ...
            final_df = pd.concat(
                [final_df, group.sort_values("채용공고수", ascending=False)[:10]]
            )
        else:
...
csdf = pd.read_csv("시장조사_240602_csdf.csv")
            ...
            children=[
                html.Div(
                    children=[
                        ...
                        dcc.Dropdown(
                            options=csdf["카테고리"].unique(), id="cate", multi=True
                        ),
                ...
                html.Div([dcc.Graph(id="result_graph")]),
            ],
            style={"marginBottom": 50, "marginTop": 50, "display": "flex"},
        ),
        html.Div(
            [
...
@callback(Output("second_addr", "options"), Input("first_addr", "value"))
def update_options(value):
    return list(
        csdf[csdf["주소(시도)"].map(lambda x: x in value)]
        .dropna(subset=["주소(시군구)"])["주소(시군구)"]
        .unique()
    )
...
def update_table(minmax, first_addr, second_addr, cate):
    if minmax[1] == 6:
        minmax[1] = max_year
    tmp_table = csdf[(csdf["최소경력"] >= minmax[0]) & (csdf["최대경력"] <= minmax[1])][
        show_cols
    ]
    #     if query:
    ...
    if conditions[0]:
        filtered_table = filtered_table[
            filtered_table["주소(시도)"].isin(
                first_addr if type(first_addr) == list else [first_addr]
            )
        ]

    if conditions[1]:
        filtered_table = filtered_table[
            filtered_table["주소(시군구)"].isin(second_addr)
        ]
...
@callback(Output("result_graph", "figure"), Input("result_table", "data"))
def update_graph(df):
    df = pd.DataFrame(df)
    df = mk_graph_data(df)
    fig = px.sunburst(
        data_frame=df, path=["주소(시도)", "카테고리", "기술스택"], values="채용공고수"
    )
    return fig
